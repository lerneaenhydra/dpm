...
consts.shaft_i = 0.3;					%ICE/generator crankshaft moment of inertia [kg*m^2]
...
%Set total battery energy for one-dimensional solution so that we could
%hypothetically do the entire trip purely electrically without depleting
%the battery
tot_p = consts.tot_p_f(consts.tt);
gross_p = tot_p + tot_p.^2 * consts.batt_quadloss;	%Instantaneous power demanded from battery if purely electric propulsion used
batt_energy = 2*sum(gross_p)*consts.T_s;					%Battery energy [J]
batt_energy = max(batt_energy, 300*60*60);	%Require total battery energy to be at least 300Wh, smaller values may give infeasibility with the chosen driving cycle
consts.batt_energy = batt_energy;			%Set total battery energy for one-dimensional solution [J]
...
if(strcmp(opmode, opmode_1d))	
	...
	consts.N_grid_soc = max(consts.N_grid_soc, 25);									%Optionally require a minimum grid density

	consts.N_grid_P_i_c_e = 25;				%Number of grid points for ICE power levels
else
	consts.N_grid_soc = 40;					%Number of grid points for SOC variable
	...
	consts.N_grid_w = max(consts.N_grid_w, 50);
	consts.N_int = 25;						%Number of integration points for BSFC/fuel consumption calculation
end
...
if(strcmp(opmode, opmode_1d))
	...
	grid_subset.range = 40 * max_shaft_energy / consts.batt_energy * ones(size(tt_1D));	%Set the search range to be sufficiently larger than the maximum energy in the crankshaft
...
if(strcmp(opmode, opmode_2d))
	...
	quantization.soc = 1 / consts.N_grid_soc;
